...
function [data_meg,...
          ...
          num,...
          timeIdx,...
          timeNew] = prepro_meg(meg,eeg,eog,whichdata)
...
if whichdata == 2        
        ...
        data_meg(:,:,megxcl) = [];        
        data_eog(:,:,megxcl) = [];
        targetSide(megxcl) = []; % removing artifacts in side of targets's tilt
        subjectRes(megxcl) = []; % removing artifacts in subject's response
        targetPos(megxcl) = []; % removing artifacts in position where the target appeared
        focusRate(megxcl) = []; % removing artifacts in participant's focus ratings
...
if whichdata == 2   
    ...
    data_eeg = 'There is no EEG dataset';
    data_eog = data_eog(:,timeIdx,:);
    num.nummegxcl = length(megxcl);   
elseif whichdata == 4
    data_meg = data_meg(:,timeIdx,:); 
    data_eeg = data_eeg(:,timeIdx,:);
    data_eog = data_eog(:,timeIdx,:);
    num.nummegxcl = length(megxcl);
    num.numeegxcl = length(eegxcl);
end

num.numAllTrial = size(meg.data,3);
num.numCorrTrial = length(corID);
num.numGutTrial = size(data_meg,3); % from good trials of eeg and meg combined
num.numBadTrial = size(meg.data,3)-num.numGutTrial;
num.numONtrial = length(idxON);
num.numMIDtrial = length(idxMID);
num.numOFFtrial = length(idxOFF);
num.numMentalTrial = length(idxMental);
