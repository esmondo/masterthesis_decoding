function [predictions,predictionsSTF,alg1,alg2] = decode_meg_2(Xmeg,Y,Z,dcodest,meg)
...
numCorID = numel(corID);
numAll = numel(meg.side);
badID = setdiff([1:numAll],corID); % use this for TESTING
numBadID = numel(badID);

%% LABEL
[Y_ori,~,~] = hemisphaereTeilen(meg.position);
[Y_corID,idxLVF,idxRVF] = hemisphaereTeilen(meg.position(corID));
numLvor = numel(idxLVF);
numRvor = numel(idxRVF);

[Yx,idxLVF,idxRVF,idxCorr] = balancetrial(Y_corID); % sample balancing
numLnach = numel(idxLVF);
numRnach = numel(idxRVF);

Yx = dcodest.Y_corbalanced;

%% EOG that needs to be rejected
Zx = dcodest.eog(:,:,dcodest.corID);
Zx = Zx(:,:,idxCorr);

%% PREDICTOR
Xmegx = meg.data(dcodest.megAOI,:,dcodest.corID); %% processing epoched data (involving correct response only)
Xmegx = Xmegx(:,:,idxCorr); %% processing epoched data (from the correct response, involve balanced trials only)
...
%%
foldIdx = mod(1:length(Y),dcodest.N); % just one possibility to define the folds
foldIDs = unique(foldIdx);
predictions = zeros(size(Y,2),dcodest.nreps);
predictionsSTF = zeros(size(Y,2),dcodest.nreps);
...
%     trainIdx = find(foldIdx~=foldIDs(k)); % you could opt to balance the class sizes if quite unbalanced
    ...
    %%
        ...
        t2 = trainIdx(Y(trainIdx) == c2); % c2 2nd class label
            ...
            for r = 1:dcodest.nreps % repetitions            
                %% Training Dataset
                    for c = 1:classSize 
                        ...
                        Xtrain(:,:,c) = mean(Xrej(:,:,t1(ridx(1:dcodest.navg))),3); 
                 
                        Ytrain(c) = c1;
                
                        ridx = randperm(length(t2));
                        Xtrain(:,:,c + classSize) = mean(Xrej(:,:,t2(ridx(1:dcodest.navg))),3);
                ...
                %% Validation Dataset
                    for c = 1:length(testIdx)
                        ...
                        Xtest(:,:,c) = mean(Xori(:,:,t3(ridx(1:dcodest.navg))),3);               
                    end
