function [predictions_x,predictionsSTF_x,alg1,alg2] = decode_meg_x(Xmeg,Y,Xmeg_x,Y_badID,Z,N,navg,nreps,meg)
% X is the predictor (meg.data)
% Y is the label (target position)
% Z is the component hat needs to be rejected (eog.data)

disp(['Begin MEG decoding algorithm (badID) of navg ', num2str(navg)])
...
Xmeg_x = double(Xmeg_x);
Xmeg_x = bandpass(1,200,meg.srate,Xmeg_x);
Xmeg_x = Xmeg_x.*1e12;
...
Xmeg_x = lowpass(12.5,meg.srate,Xmeg_x);
...
[ds_Xmeg_x,t_Xmeg_x] = dwsample(size(Xmeg_x,1),FsNew,meg.srate,Xmeg_x,meg.time); 

% Baseline Correction
Xrej = baseline(ds_Xrej,t_Xrej);
Xori = baseline(ds_Xori,t_Xori);
Xmeg_x = baseline(ds_Xmeg_x,t_Xmeg_x);
...
%                         
%                         try
                            Xtest(:,:,c) = mean(Xmeg_x(:,:,t3(ridx(1:navg))),3); % bad data for testing
%                         catch 
%                             dbstop if error
%                         end
...
%                     Xtest = Xori(:,:,testIdx_x);
                    Xtest = Xmeg_x(:,:,testIdx_x); % bad data for testing
